...
module psram_controller #(
    ...
    input  wire        clk,
    input  wire        clk_p,       // phase-shifted clock for driving O_psram_ck
    input  wire        resetn,
    output wire        resetn_o,    // 1 after config done, use as system reset for the rest of the system
    input  wire        read,        // Set to 1 to read from RAM
    input  wire        write,       // Set to 1 to write to RAM
    input  wire [21:0] addr,        // Byte address to read / write
    input  wire [15:0] din,         // Data word to write
    input  wire        byte_write,  // When writing, only write one byte instead of the whole word. 
                              // addr[0]==1 means we write the upper half of din. lower half otherwise.
    input  wire        rdv,         // vram 32-bit aligned read
    output reg  [15:0] dout,   // Last read data. Read is always word-based.
    output reg  [15:0] dout2,  // second wrod
    output wire        busy,   // 1 while an operation is in progress
...
always @(posedge clk) begin
    ...
    if (state == IDLE_ST) begin
        ...
        begin
            ...
            // Linear burst keeps the second rdv word at addr+2 instead of wrapping unexpectedly.
            dq_sr <= {read_combined ? 1'b1 : write_combined ? 1'b0 : 1'b1, 13'b010_0000_0000_00, addr[21:4], 13'b0, addr[3:1], 16'b0000_0100_1101_0100};
